...
const BUTTONID_BUY_SABOTAGECOMMS = "league:dm:buy:abotageComms";
const BUTTONID_BUY_ASSASSINATE = "league:dm:buy:assassinate";
const BUTTONID_BUY_CONFIRMEJECTION = "league:dm:buy:confirmEjection";
const BUTTONID_BUY_SHERIFF = "league:dm:buy:Sheriff";
...
export class Game {
  ...
  async handleButton(client, interaction) {
    ...
    if (customId.startsWith("league:announcement:")) {
      interactionObject = new InteractionAnnouncement();
    } else if (customId.startsWith("league:dm:buy:")) {
      interactionObject = new InteractionDirectMessage();
    } else {
    ...
    interactionObject.game = this;
    interactionObject.client = client;
    ...
    let host = this.client.users.cache.get(hostId);
    if (!host) {
      host = await this.client.users.fetch(hostId).catch(() => null);
    }
    if (!host) {
      await interactionReply(interaction, "Host user not found.");
      return;
    }
    this.host = host;
  ...
  async resolvePlayerIds() {
    ...
    for (const playerId of this.playerIds) {
      ...
      if (!player) {
        player = await this.client.users.fetch(playerId).catch(() => null);
      }
      if (!player) {
...
class InteractionAnnouncement {
  async init() {
    const fields = this.interaction.message.embeds[0].fields;
    let statusField = fields.find((field) => field.name === "Status");
    this.game.status = statusField.value;

    let playerCountField = fields.find(
      (field) => field.name === "Player Count"
    );
    this.game.playerCount = parseInt(playerCountField.value);

    let imposterCountField = fields.find(
      (field) => field.name === "Imposter Count"
    );
    this.game.imposterCount = parseInt(imposterCountField.value);

    const signedUpPlayers = fields.filter(
      (field) =>
        field.name.startsWith("Player") && field.name !== "Player Count" && field.value !== "open"
    );
    this.game.playerIds = [];
  ...
  async signup() {
    ...
    if (this.game.host.id === `${this.interaction.user.id}` && isProduction && !isDev) {
    ...
    const alreadySignedUp = this.game.playerIds.includes(`${this.interaction.user.id}`);
    ...
    if (this.game.playerIds.length >= this.game.playerCount) {
      await interactionReply(this.interaction, "No open slots available.");
      return;
    }
    // Register the user as a player
    this.game.playerIds.push(`${this.interaction.user.id}`);
    // check if the game is now full
    let gameFull = this.game.playerIds.length >= this.game.playerCount;
    if (gameFull) {
      this.game.status = "Ready";
    }
    const newButtons = await this.game.announcementBuildButtons();
    const newEmbeds = await this.game.announcementBuildEmbed();
    ...
    if (gameFull) {
      await this.game.host.send(`Game ID ${this.game.gameId} is ready to start!`);
    }
  }
  async signupCancel() {
    ...
    if (this.game.status !== "Setup" && this.game.status !== "Ready") {
    ...
    this.game.playerIds = this.game.playerIds.filter(
    ...
    if (oldStatus === "Ready") {
      await this.game.host.send(`Player ${this.interaction.user.username} has cancelled their sign up for game ID ${this.game.gameId}. The game is no longer ready to start.`);
    }
  ...
  async gameStart() {
    ...
    if (this.game.host.id !== `${this.interaction.user.id}`) {
    ...
    await this.game.resolvePlayerIds();
    // roll one or two players to be imposters randomly, this.imposterCount
    let imposter1 = Math.floor(Math.random() * this.game.playerIds.length);
    let imposter2;
    if (this.game.imposterCount === 2) {
      do {
        imposter2 = Math.floor(Math.random() * this.game.playerIds.length);
      } while (imposter1 === imposter2);
    }
    this.game.imposter1 = this.game.players[imposter1];
    this.game.imposter2 = null;
    if (this.game.imposterCount === 2) {
      this.game.imposter2 = this.game.players[imposter2];
    }
    // for now lets roll credits randomly
    //this.rollCredits();
    // send a message to the players with their roles
    this.game.players.forEach((player) => {
      this.game.sendStartToPlayer(player);
    });
    // send a message to the host with the game details
    this.game.sendStartToHost();
    // update the embed
    let statusField = message.embeds[0].fields.find((field) => field.name === "Status");
  ...
  async gameCancel() {
    ...
    // check that the game is not already cancelled or started
    if (this.game.status !== "Ready" && this.game.status !== "Setup") {
...
class InteractionDirectMessage {
